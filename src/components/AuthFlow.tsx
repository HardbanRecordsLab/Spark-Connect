...
function OnboardingView({ onComplete }: { onComplete: () => void }) {
  ...
  const steps = [
    {
      title: 'Dodaj zdjęcie 📸',
      subtitle: 'Pierwsze wrażenie ma znaczenie — możesz dodać więcej w profilu',
      valid: true,
      content: (
        <div className="flex flex-col items-center gap-4">
          <div
            onClick={() => photoInputRef.current?.click()}
            className="relative w-44 h-44 rounded-full overflow-hidden cursor-pointer border-2 border-dashed border-primary/50 hover:border-primary transition-all flex items-center justify-center bg-secondary/30"
          >
            {avatarPreview
              ? <img src={avatarPreview} alt="" className="w-full h-full object-cover" />
              : <div className="text-center p-4">
                  <Camera className="w-12 h-12 text-muted-foreground mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground">Dotknij aby dodać</p>
                </div>
            }
            <div className="absolute bottom-2 right-2 w-9 h-9 gradient-fire rounded-full flex items-center justify-center shadow-lg">
          ...
          <input ref={photoInputRef} type="file" accept="image/*" capture="user"
            ...
            className="hidden" />
          {avatarPreview && (
            <button onClick={() => { setAvatarFile(null); setAvatarPreview(''); }} className="text-xs text-muted-foreground underline">
              Zmień zdjęcie
            </button>
          )}
          <p className="text-xs text-muted-foreground text-center opacity-60">Możesz pominąć — dodaj zdjęcie później</p>
        </div>
      ),
    },
    {
      title: 'Imię i wiek 👋',
      subtitle: 'Jak masz na imię i ile masz lat?',
      valid: data.display_name.trim().length >= 2 && parseInt(data.age) >= 18,
      content: (
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full glass rounded-2xl px-4 py-4 text-lg outline-none border border-border focus:border-primary transition-colors"
              autoFocus
            />
          </div>
          <div>
    ...
    {
      title: 'Jestem... 🧬',
      subtitle: 'Jak się identyfikujesz i kogo szukasz?',
      valid: !!data.gender && !!data.orientation,
      content: (
        <div className="space-y-4">
          <div>
            <p className="text-sm text-muted-foreground mb-2">Jestem...</p>
            <div className="grid grid-cols-2 gap-2">
              {['Mężczyzna 🕺', 'Kobieta 💃', 'Niebinarny/a ✨', 'Inne'].map(g => (
                <button key={g} onClick={() => setData({ ...data, gender: g })}
                  className={`py-3.5 rounded-2xl font-semibold text-sm transition-all ${data.gender === g ? 'gradient-fire text-primary-foreground' : 'glass border border-border'}`}>
                  {g}
                </button>
              ))}
            </div>
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-2">Interesuję się...</p>
            <div className="space-y-2">
              {[
                { v: 'women', l: 'Kobietami 💋' },
                { v: 'men',   l: 'Mężczyznami 🔥' },
                { v: 'all',   l: 'Każdym — jestem otwarty/a 💫' },
              ].map(o => (
                <button key={o.v} onClick={() => setData({ ...data, orientation: o.v })}
                  className={`w-full py-3.5 rounded-2xl font-semibold text-sm transition-all ${data.orientation === o.v ? 'gradient-fire text-primary-foreground' : 'glass border border-border'}`}>
                  {o.l}
                </button>
              ))}
            </div>
    ...
    {
      title: 'Czego szukasz? 🎯',
      subtitle: 'Powiedz nam co Cię kręci',
      valid: !!data.relationship_type,
      content: (
        <div className="space-y-3">
          {[
            { v: 'hookup',       l: 'Seks bez zobowiązań 🔥',    sub: 'Dyskretnie, bez dramatów' },
            { v: 'friends',      l: 'Seks z przyjacielem 😈',    sub: 'Znajomość z korzyściami' },
            { v: 'relationship', l: 'Stały partner/ka 💍',       sub: 'Czegoś poważniejszego' },
            { v: 'both',         l: 'Wszystkiego spróbuję ✨',   sub: 'Niech los zdecyduje' },
          ].map(o => (
            <button key={o.v} onClick={() => setData({ ...data, relationship_type: o.v })}
              className={`w-full py-3.5 px-4 rounded-2xl text-left transition-all ${data.relationship_type === o.v ? 'gradient-fire text-primary-foreground' : 'glass border border-border'}`}>
  ...
  return (
    <div className="min-h-screen bg-radial-glow flex flex-col px-6 pt-10 pb-8">
      {/* Progress bar */}
      <div className="mb-2">
        ...
        <p className="text-xs text-muted-foreground mt-1.5 text-right">
          Krok {step + 1} z {steps.length} — tylko chwila!
        </p>
      </div>
      ...
      <div className="flex gap-3 pt-4">
        {step > 0 && (
          <button onClick={() => setStep(s => s - 1)} className="px-6 py-4 glass rounded-2xl font-semibold">
            ← Wróć
          </button>
        )}
        <button
          ...
          className="flex-1 gradient-fire text-primary-foreground font-bold py-4 rounded-2xl glow-red disabled:opacity-40 flex items-center justify-center gap-2 transition-opacity"
        >
          {loading
      ...
      {!isLast && (
        <button
          ...
          disabled={loading}
          className="text-center text-xs text-muted-foreground/50 mt-3 hover:text-muted-foreground transition-colors"
        >
          Pomiń i uzupełnij profil później
        </button>
      )}
    </div>
  );
